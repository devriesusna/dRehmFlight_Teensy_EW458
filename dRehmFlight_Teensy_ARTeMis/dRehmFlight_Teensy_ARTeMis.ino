...
//Version: Beta 1.31
//Modified for general ARTeMis use by L. DeVries
...
BNO-055 implementation based on BNO-055 library by:

Mavlink implementation based on mavlink library by:
...
#include <Adafruit_BNO08x.h> //IMU library I2C communication 
//#include "src/Adafruit_DotStar/Adafruit_DotStar.h" //Dotstar LED string library
#include <Adafruit_DotStar.h> //Dotstar LED string library
  ...
  #include "src/DSMRX/DSMRX.h"  //serial DSMX interface
#endif
...
float Kp_pos = 0.0;         // waypoint P-gain
float Ki_pos = 0.0;         // waypoint I-gain
float Kd_pos = 0.0;         // waypoint D-gain
...
const int m2Pin = 1;
const int m3Pin = 2;
const int ledDi = 3; // lightbar pin
const int m5Pin = 4;
const int ledCl = 5; // lightbar pin
...
bool blinkAlternate;
bool ledsOn;
// Color definitions for the lightbar
int red = 0xFF;
int yellow = 0xFFFF;
int green = 0xFF00;
int blue = 0xFF0000;
...
float q2 = 0.0f;
float q3 = 0.0f;
float m = 1; // mass (kg) UPDATE VALUE
...
// Position Controller
float error_x, error_y, error_z; // position error variables
float x_pos, y_pos, z_pos; // position variables (from MAVLINK Serial)
float x_des, y_des, z_des; // desired position (from MAVLINK Serial)
float integral_x, integral_y, integral_z; // integral control variable
float integral_x_prev, integral_y_prev, integral_z_prev; // aging integral term
float derivative_x, derivative_y, derivative_z; // used to show derivative term
float error_x_prev, error_y_prev, error_z_prev; // aging variable for error derivative
float x_PID, y_PID, z_PID; // PID outputs for position controller (send to desired angles)
...
// BNO-085 IMU sensor (replaces MPU6050 used in initial dRehm distribution)
// Adafruit_BNO08x  bno08x(BNO08X_RESET);
sh2_SensorValue_t sensorValue;
// Lightbar used to convey flight modes and fault codes
...
void setup() {
  ...
  Serial5.begin(115200); // Serial connection to MAVLINK streaming device (RPi or Pico)
  delay(500);
...
void loop() {
  ...
  // Receive mavlink serial messages
  receiveSerial();
...
void updateStatus() {
  ...
  if(channel_6_pwm >=1500){
    guided = true; // add logic here to put it into position mode if ch6 is 3 position
  }else{
    guided = false;
  }
...
void getDesState() {
  ...
  if(guided){
    ...
    // uses PID position controller on altitude for throttle, x-position for pitch, y-position for roll
    controlPOSITION();
    thro_des = constrain(z_PID,0.0,1.0);
    roll_des = constrain(y_PID,-maxRoll,maxRoll);
    pitch_des = constrain(x_PID,-maxPitch,maxPitch);
    yaw_des = constrain(yaw_cmd,-maxYaw,maxYaw); // yaw command pass through
...
// X-POSITION
  error_x = x_des - x_pos;
  integral_x = integral_x_prev + error_x*dt;
  if (channel_1_pwm < throt_lim) {   //Don't let integrator build if throttle is too low
     integral_x = 0;
     integral_y = 0;
     integral_z = 0;
  ...
  integral_x = constrain(integral_x, -i_limit, i_limit); //Saturate integrator to prevent unsafe buildup
  derivative_x = (error_x-error_x_prev)/dt;
  x_PID = (Kp_pos*error_x + Ki_pos*integral_x - Kd_pos*derivative_x); //Scaled by .01 to bring within -1 to 1 range

  // Y-POSITION
  error_y = y_des - y_pos;
  integral_y = integral_y_prev + error_y*dt;
  integral_y = constrain(integral_y, -i_limit, i_limit); //Saturate integrator to prevent unsafe buildup
  derivative_y = (error_y-error_y_prev)/dt;
  y_PID = (Kp_pos*error_y + Ki_pos*integral_y - Kd_pos*derivative_y); //Scaled by .01 to bring within -1 to 1 range

  // Z-Position (ALTITUDE)
  error_z = z_des - z_pos;
  integral_z = integral_z_prev + error_z*dt;
  integral_z = constrain(integral_z, -i_limit, i_limit); //Saturate integrator to prevent unsafe buildup
  derivative_z = (error_z - error_z_prev)/dt; 
  z_PID = 1/(cos(roll_IMU)*cos(pitch_IMU))*(Kp_pos*error_z + Ki_pos*integral_z + Kd_pos*derivative_z); //Scaled by .01 to bring within -1 to 1 range

  //Update x variables
  integral_x_prev = integral_x;
  error_x_prev = error_x;
  //Update y variables
  integral_y_prev = integral_y;
  error_y_prev = error_y;
  //Update z variables
  error_z_prev = error_z;
  integral_z_prev = integral_z;
}
...
void receiveSerial(){
//DESCRIPTION: receive mavlink2 messages from a connected serial comms device
  /*
      Parses mavlink messages from serial device. allowed messages to date:
      ...
      if(mavlink_parse_char(MAVLINK_COMM_0,c,&msg,&status)){
          ...
          switch(msg.msgid){
            ...
            case MAVLINK_MSG_ID_SET_POSITION_TARGET_LOCAL_NED:
            {
                ...
                // Access parsed data
                x_des = pos_target.x; // desired position x
                y_des = pos_target.y; // desired position y
                z_des = pos_target.z; // desired position z
                yaw_des = pos_target.yaw; // desired yaw, rad

                // Example: Print the parsed data
                printf("Parsed SET_POSITION_TARGET_LOCAL_NED message: x_des=%f, y_des=%f, z_des=%f\n", x_des, y_des, z_des);
            }
            case MAVLINK_MSG_ID_COMMAND_LONG:
            {
              ...
              int val = mode.base_mode;
              printf("Parsed SET_MODE message: mode=%d\n", val);
            }
            case MAVLINK_MSG_ID_LOCAL_POSITION_NED:
            {
              ...
              y_pos = pos.y;
              z_pos = pos.z;
              printf("Parsed LOCAL_POSITION__NED message: x=%f, y=%f, z=%f\n", x_pos, y_pos, z_pos);
            }
          }//switch
...
void receiveSerial2(){
//DESCRIPTION: receive messages from a connected serial comms device
  ...
  if(Serial5.available()>0){ 
      ...
      // Parse the received data into individual float values
      float float_values[4];
      int index = 0;
      char* token = strtok(const_cast<char*>(data.c_str()), ",");
      while (token != NULL && index < 3) {
        float_values[index++] = atof(token);
        token = strtok(NULL, ",");
      }
      switch(int(float_values[0])){
        case 0: // MoCap position message
          x_pos = float_values[0];
          y_pos = float_values[1];
          z_pos = float_values[2];
        case 1: // desired position command message
          x_des = float_values[0];
          y_des = float_values[1];
          z_des = float_values[2];
      }
